#!/usr/bin/env python3
"""
Pi Dashboard — Cross-Platform Setup Wizard
===========================================
...
def _systemd_user(cfg: dict) -> dict:
    ...
    unit_file = unit_dir / 'pi-dashboard.service'

    unit_file.write_text(f"""[Unit]
Description=Pi Command Dashboard
...
""")
    ...
    for cmd in [
        ...
        ['systemctl', '--user', 'enable', 'pi-dashboard'],
...
def _linux_shortcuts(url: str, cfg: dict) -> list[str]:
    ...
    if desktop.exists():
        p = desktop / 'pi-dashboard.desktop'
...
<div class="logo">pi-dashboard &mdash; setup wizard</div>
...
<div class="card">
  ...
  <div class="step active" id="s1">
    <h1>Welcome</h1>
    <p class="sub">This wizard configures Pi Dashboard on your machine. It takes about a minute and requires no elevated permissions.</p>
    <div class="chips">
  ...
  <div class="step" id="s6">
    ...
    <p class="sub">Pi Dashboard is installed and ready. Click the URL below to open it, or run <code>python3 dashboard.py</code> manually.</p>
...
function renderInfo() {
  ...
  const cmdsMap = {
    Linux:   'systemctl --user status pi-dashboard\nsystemctl --user stop    pi-dashboard\nsystemctl --user start   pi-dashboard\njournalctl --user -u pi-dashboard -f',
    Darwin:  'launchctl list | grep pidashboard\nlaunchctl stop  com.pidashboard.app\nlaunchctl start com.pidashboard.app\ncat ~/Library/Logs/pi-dashboard.log',
    Windows: 'schtasks /query /tn PiDashboard\ntaskkill /f /im python.exe\n# Or open Task Scheduler → PiDashboard',
  };
...
def main():
    ...
    print()
    print('─' * 54)
    print('  Pi Dashboard — Setup Wizard')
